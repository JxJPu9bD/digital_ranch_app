...
class DiseaseHealthView extends GetView<DiseaseHealthController> {
  const DiseaseHealthView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('疾病健康'),
        leading: IconButton(
  ...
  Widget _buildTabViews() {
    return TabBarView(
      controller: controller.tabController,
      children: [
        _buildList(controller.newDiseaseList),
        _buildList(controller.treatmentList),
        _buildList(controller.outcomeList),
        _buildList(controller.quarantineList),
      ],
    );
  }
              ...
              children: [
                Row(
                  children: [
                    ...
                    SizedBox(width: 8.w),
                    Text(item['disease'] ?? '', style: TextStyle(fontSize: 14.sp, color: Colors.black87)),
                  ],
                ),
                SizedBox(height: 6.h),
                Row(
