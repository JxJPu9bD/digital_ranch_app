...
class BreedingBusinessView extends GetView<BreedingBusinessController> {
  const BreedingBusinessView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('繁育业务'),
        leading: IconButton(
  ...
  Widget _buildTabViews() {
    final listBuilders = [
      () => _buildList(controller.estrusList),
      () => _buildList(controller.breedingList),
      () => _buildList(controller.firstCheckList),
      () => _buildList(controller.recheckList),
      () => _buildList(controller.abortionList),
      () => _buildList(controller.banUnbanList),
    ];
    return TabBarView(
      controller: controller.tabController,
      children: listBuilders.map((b) => b()).toList(),
    );
  }
